...
export const idMap = headings.reduce((acc, current, i) => {
  current.id = i;
  acc[i] = {};
  acc[i].id = i;

  if (prev) {
    parent = compareTags(current, prev);
  }
  ...
  acc[i].parentElement = parent;
  const parentId = +(parent && parent.id) || null;
  acc[i].parentId = parentId;
  acc[i].element = current;
  current.dataset.parentId = parentId;
  prev = current;
  return acc;
}, {});

function compareTags(el, prev) {
  if (prev.tagName < el.tagName) {
    return prev;
  } else if (prev.tagName === el.tagName) {
    return parent;
  } else {
    return el;
  }
